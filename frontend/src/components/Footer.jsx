...
export default function Footer() {
  ...
  return (
    <footer className="bg-[#fff9f0] border-t border-[#fde6c8] mt-16" data-testid="site-footer">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-12 grid md:grid-cols-3 gap-10">
        ...
        <div>
          ...
          <form onSubmit={submit} className="flex gap-2" data-testid="footer-signup-form">
            <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com"
              className="flex-1 px-4 py-3 rounded-full border-2 border-[#fde6c8] bg-white focus:outline-none focus:border-[#40e0d0]" data-testid="footer-signup-input" />
            <button disabled={busy} className="btn-primary" data-testid="footer-signup-submit">{busy ? "..." : "Join"}</button>
          </form>
          <div className="flex gap-3 mt-5">
            {socials.map((s) => (
              <a key={s.label} href={s.url} target="_blank" rel="noopener noreferrer" aria-label={s.label} className="w-10 h-10 rounded-full bg-white border-2 border-[#fde6c8] grid place-items-center hover:border-[#40e0d0] transition" data-testid={`footer-social-${s.label.toLowerCase()}`}>
                <s.icon className="w-5 h-5 text-[#2e8b57]" />
              </a>
            ))}
          </div>
        </div>
        <div className="text-sm text-[#6b7280]">
          ...
          <ul className="space-y-2">
            <li><a className="hover:text-[#2e8b57]" href="/story">Meet the Sea Stars</a></li>
            <li><a className="hover:text-[#2e8b57]" href="/downloads">Free Downloads</a></li>
            <li><a className="hover:text-[#2e8b57]" href="/for-camps">For Camp Directors</a></li>
            <li><a className="hover:text-[#2e8b57]" href="/contact">Contact</a></li>
          </ul>
        </div>
      </div>
      <div className="border-t border-[#fde6c8] py-4 text-center text-xs text-[#6b7280]" data-testid="footer-copyright">{site.footer_text || "© 2026 KingApe Media. All rights reserved."}</div>
    </footer>
  );
}
