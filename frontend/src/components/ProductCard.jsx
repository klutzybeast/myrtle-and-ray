      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2">
          ...
          {p.inventory_status && p.inventory_status !== "In Stock" && (
            <span className="text-[10px] uppercase tracking-wider font-bold bg-[#fff5ec] text-[#e89bab] px-2 py-1 rounded-full">{p.inventory_status}</span>
          )}
        </div>
        ...
        <div className="mt-3 flex items-end justify-between">
          <div>
            ...
            <span className="font-accent text-xl font-bold text-[#5a8a6f]" data-testid={`product-price-${p.slug}`}>${(p.price || 0).toFixed(2)}</span>
          </div>
