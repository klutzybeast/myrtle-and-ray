...
export default function DownloadCard({ d, characterImage }) {
  return (
    <Link to={`/downloads/${d.slug}`} className="card-soft block overflow-hidden group" data-testid={`download-card-${d.slug}`}>
      <div className="aspect-[4/3] bg-[#e0f7fa] relative overflow-hidden">
        {d.cover_image ? <img src={d.cover_image} alt={d.title} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition" /> : <div className="w-full h-full grid place-items-center text-[#87ceeb] font-accent text-2xl"><FileText className="w-12 h-12" /></div>}
        {d.is_new && <span className="absolute top-3 left-3 bg-[#ff9b71] text-white text-[10px] uppercase tracking-wider font-bold px-2 py-1 rounded-full" data-testid="download-new-badge"><Sparkles className="w-3 h-3 inline mr-1" />New</span>}
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-1 mt-3">
          {(d.audiences || []).slice(0, 3).map((a) => (
            <span key={a} className="text-[10px] font-bold uppercase tracking-wider bg-[#e0f7fa] text-[#2e8b57] px-2 py-1 rounded-full">{a}</span>
          ))}
        </div>
