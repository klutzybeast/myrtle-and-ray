...
export default function ShopDetail() {
  ...
  return (
    <main className="pt-24 pb-12 bg-[#fff9f0] min-h-screen" data-testid="product-detail-page">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          <div>
            <div className="aspect-square bg-[#e0f7fa] rounded-[28px] overflow-hidden mb-3">
              {images[activeImage] && <img src={images[activeImage]} alt={p.name} className="w-full h-full object-cover" />}
            </div>
            {images.length > 1 && (
              <div className="flex gap-2 overflow-x-auto no-scrollbar">
                {images.map((img, i) => (
                  <button key={i} onClick={() => setActiveImage(i)} className={`flex-shrink-0 w-20 h-20 rounded-2xl overflow-hidden border-2 ${activeImage === i ? "border-[#40e0d0]" : "border-transparent"}`} data-testid={`thumb-${i}`}>
          ...
          <div>
            ...
            {p.character_slug && (
              <Link to={`/story#${p.character_slug}`} className="inline-block mt-2 text-sm text-[#3cb371] font-semibold" data-testid="product-character-chip">
            ...
            <div className="mt-3 flex items-baseline gap-3">
              ...
              <span className="font-accent text-4xl font-bold text-[#2e8b57]">${(p.price || 0).toFixed(2)}</span>
              {p.inventory_status && p.inventory_status !== "In Stock" && (
                <span className="text-xs uppercase tracking-wider font-bold bg-[#fff5f0] text-[#ff6f91] px-3 py-1 rounded-full">{p.inventory_status}</span>
              )}
            </div>
            ...
            {(p.variants || []).length > 0 && (
              <div className="mt-4">
                ...
                <select value={variantIdx} onChange={(e) => setVariantIdx(parseInt(e.target.value, 10))} className="block mt-1 px-4 py-3 rounded-full border-2 border-[#fde6c8] bg-white font-semibold" data-testid="variant-select">
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <input type="number" min="1" value={qty} onChange={(e) => setQty(parseInt(e.target.value || "1", 10))} className="w-20 px-3 py-2 rounded-full border-2 border-[#fde6c8] text-center" data-testid="product-qty" />
            </div>
