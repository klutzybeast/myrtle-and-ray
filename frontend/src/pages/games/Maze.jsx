...
// Recursive backtracking maze (cells x cells). Walls live BETWEEN cells.
function generateMaze(w, h) {
  const cells = Array.from({ length: h }, () =>
    Array.from({ length: w }, () => ({ N: true, S: true, E: true, W: true, visited: false }))
  );
  const stack = [[0, 0]];
...
export default function Maze({ data, onComplete }) {
  const w = Math.min(Math.max(parseInt(data?.width || 12, 10), 5), 20);
  const h = Math.min(Math.max(parseInt(data?.height || 12, 10), 5), 20);
      ...
      <div ref={boardRef} className="mx-auto rounded-2xl p-2 bg-[#eef9fb] border-4 border-white shadow-inner" style={{ width: "fit-content" }} tabIndex={0}>
        <svg width={w * SIZE} height={h * SIZE} viewBox={`0 0 ${w * SIZE} ${h * SIZE}`}>
          {maze.flatMap((row, y) => row.map((cell, x) => (
            <g key={`${x}-${y}`}>
              {cell.N && <line x1={x * SIZE} y1={y * SIZE} x2={(x + 1) * SIZE} y2={y * SIZE} stroke="#3a4a55" strokeWidth="3" strokeLinecap="round" />}
              {cell.W && <line x1={x * SIZE} y1={y * SIZE} x2={x * SIZE} y2={(y + 1) * SIZE} stroke="#3a4a55" strokeWidth="3" strokeLinecap="round" />}
              {y === h - 1 && cell.S && <line x1={x * SIZE} y1={(y + 1) * SIZE} x2={(x + 1) * SIZE} y2={(y + 1) * SIZE} stroke="#3a4a55" strokeWidth="3" strokeLinecap="round" />}
              {x === w - 1 && cell.E && <line x1={(x + 1) * SIZE} y1={y * SIZE} x2={(x + 1) * SIZE} y2={(y + 1) * SIZE} stroke="#3a4a55" strokeWidth="3" strokeLinecap="round" />}
            </g>
          )))}
          {/* goal */}
          <text x={(w - 1) * SIZE + SIZE / 2} y={(h - 1) * SIZE + SIZE * 0.72} textAnchor="middle" fontSize={SIZE * 0.7}>🐚</text>
          {/* player */}
          <text x={pos.x * SIZE + SIZE / 2} y={pos.y * SIZE + SIZE * 0.72} textAnchor="middle" fontSize={SIZE * 0.7} data-testid="maze-player">🐳</text>
        </svg>
      </div>
