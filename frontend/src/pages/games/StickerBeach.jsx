...
export default function StickerBeach({ data, onComplete }) {
  const sceneImg = data?.scene_image || DEFAULT_SCENE;
  const [items, setItems] = useState([]); // [{id, emoji, x, y, size}]
  ...
  const addAt = (e) => {
    ...
    setItems((arr) => {
      const next = [...arr, { id: newId, emoji: picked, x, y, size: 44 }];
      if (next.length === 5) onComplete?.();
      return next;
    });
  };
  ...
  return (
    <div className="space-y-3" data-testid="game-sticker-beach">
      ...
      <div
        ...
        style={{ aspectRatio: "16/9", background: `center/cover no-repeat url(${sceneImg})` }}
        data-testid="sticker-stage"
      >
        {items.map((it) => (
          <div
            key={it.id}
            onMouseDown={(e) => onDown(it.id, e)}
            onTouchStart={(e) => onDown(it.id, e)}
            onDoubleClick={(e) => removeOne(it.id, e)}
