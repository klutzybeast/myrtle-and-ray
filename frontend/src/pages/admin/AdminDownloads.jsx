    ...
    <div data-testid="admin-downloads">
      ...
      <div className="bg-white rounded-3xl border border-[#fde6c8] overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.map((d) => (
              <tr key={d.slug} className="border-t border-[#fde6c8]">
                ...
                <td className="p-3"><span className={`text-xs font-bold ${d.published ? "text-[#3cb371]" : "text-[#6b7280]"}`}>{d.published ? "Live" : "Draft"}</span>{d.featured && <span className="ml-2 text-[#ff9b71]">★</span>}{d.is_new && <span className="ml-2 text-xs bg-[#ff9b71] text-white px-2 rounded-full">NEW</span>}</td>
    ...
    <div className="fixed inset-0 z-50 bg-black/40 grid place-items-center p-4" onClick={onCancel}>
      <div className="bg-white rounded-[24px] max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()} data-testid="download-editor">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <F label="Categories" full><div className="flex flex-wrap gap-2">{cats.map((c) => <button type="button" key={c.slug} onClick={() => toggleArr("category_slugs", c.slug)} className={`px-3 py-1 rounded-full text-xs font-bold ${(item.category_slugs || []).includes(c.slug) ? "bg-[#40e0d0] text-white" : "bg-[#fff9f0] text-[#4a5568]"}`}>{c.name}</button>)}</div></F>
          <F label="Audiences" full><div className="flex flex-wrap gap-2">{AUDIENCES.map((a) => <button type="button" key={a} onClick={() => toggleArr("audiences", a)} className={`px-3 py-1 rounded-full text-xs font-bold ${(item.audiences || []).includes(a) ? "bg-[#ff9b71] text-white" : "bg-[#fff9f0] text-[#4a5568]"}`}>{a}</button>)}</div></F>
          <F label="W.A.V.E. values" full><div className="flex gap-2">{WAVE.map((w) => <button type="button" key={w} onClick={() => toggleArr("wave_values", w)} className={`w-10 h-10 rounded-full font-bold ${(item.wave_values || []).includes(w) ? "bg-[#3cb371] text-white" : "bg-[#fff9f0] text-[#4a5568]"}`}>{w}</button>)}</div></F>
