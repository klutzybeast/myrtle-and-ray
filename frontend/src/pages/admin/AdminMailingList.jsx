  ...
  return (
    <div data-testid="admin-mailing">
      ...
      <form onSubmit={add} className="card-soft p-4 mb-4 flex flex-wrap gap-2 items-end">
        <input placeholder="email" value={email} onChange={(e) => setEmail(e.target.value)} required type="email" className="px-4 py-2 rounded-full border-2 border-[#fde6c8] flex-1 min-w-[200px]" />
        <input placeholder="name" value={name} onChange={(e) => setName(e.target.value)} className="px-4 py-2 rounded-full border-2 border-[#fde6c8] flex-1 min-w-[200px]" />
      ...
      <div className="bg-white rounded-3xl border border-[#fde6c8] overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-[#fff9f0] text-left"><tr><th className="p-3">Email</th><th className="p-3">Name</th><th className="p-3">Source</th><th className="p-3">Tags</th><th></th></tr></thead>
          <tbody>
            {items.map((m) => (
              <tr key={m.id} className="border-t border-[#fde6c8]">
