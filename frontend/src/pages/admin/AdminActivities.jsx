    ...
    <div data-testid="admin-activities">
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="space-y-2">
          {items.map((p) => (
            <button key={p.key} onClick={() => setActive(p)} className={`block w-full text-left p-3 rounded-2xl border-2 ${active?.key === p.key ? "border-[#7fcfc7] bg-[#eef9fb]" : "border-[#f4e4c6] bg-white"}`}>
        ...
        <div className="lg:col-span-2">
          {active ? (
            <div className="card-soft p-5">
              ...
              <textarea value={text} onChange={(e) => setText(e.target.value)} rows={20} className="w-full p-4 rounded-2xl border-2 border-[#f4e4c6] font-mono text-sm focus:outline-none focus:border-[#7fcfc7]" />
